 ...
 *
 * 原始html文件路径：/resrouces/app/type/type.html，文件夹名要和html名一致
 *
...
async function render1(app = "", type = "", data = {}, imgType = "jpeg") {
  if (lodash.isUndefined(data._res_path)) {
    data._res_path = `../../../../../plugins/python-plugin/resrouces/`;
  }
  if (lodash.isUndefined(data._sys_res_path)) {
    data._sys_res_path = `../../../../../plugins/python-plugin/resrouces/`;
  }
  let tplKey = `${app}.${type}`;
  let saveId = data.save_id || type;
  let tplFile = `${_path}/plugins/python-plugin/resrouces/${app}/${type}.html`;
  Data.createDir(_path + `/data/`, `html/plugin_python-plugin/${app}/${type}`);
  let savePath = _path + `/data/html/plugin_python-plugin/${app}/${type}/${saveId}.html`;
