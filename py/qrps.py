...
CAILIAODIAN_PATH=os.path.join(FILE_PATH,'cailiaodian')
Image.MAX_IMAGE_PIXELS = None
if not os.path.exists(CAILIAODIAN_PATH):
    os.makedirs(CAILIAODIAN_PATH)
CENTER = None
MAP_ICON = None
...
def up_label_and_point_list():
    ...
    label_data =download_json(LABEL_URL)
    ...
    test =download_json(POINT_LIST_URL)
...
class Resource_map(object):
    ...
    def get_cq_cod(self):
        ...
        self.map_image.save(CAILIAODIAN_PATH+'/'+self.name+'.jpg')
        base64_str =base64.b64encode(bio.getvalue()).decode()

        return base64_str#f"[CQ:image,file={base64_str}]"
...
def draw_help():
    a=get_resource_list_mes()
    lot_font = ImageFont.truetype(FONT_PATH, 25)
    bg = Image.new("RGB",(1350,3000),(255,255,255))
    pen = ImageDraw.Draw(bg)
    x=50
    y=50
    n=0
    temp = {}
    for i in data["all_resource_type"].keys():
        if data['all_resource_type'][i]["depth"]==2:
            pen.text((x, y),data['all_resource_type'][i]['name'], fill="#8d7650ff", font=lot_font)
            x=x+250
            if x>250*4+50:
                y=y+lot_font.size*1.5
                x=50
    bg1=bg.crop((0,0,1350,int(y+lot_font.size*1.5)))
...
if __name__=="__main__":
    ...
    #a=bytes('清心',encoding='utf-8')
    #from urllib.parse import quote
    #a=quote('清心','utf-8')
    #base64_str =base64.b64encode('清心'.encode('utf8'))#.decode()
    #print(a)
